...
const UpdateCorpBenefits = () => {
  ...
  const [selectedBenefits, setSelectedOptions] = useState(benefitOptions.map(option => ({ label: option.label, value: option.value })));
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setSelectedOptions(data.selectedBenefits
        );
...
//   }, []);
  ...
  const handleSubmit = () => {
    ...
    dispatch({ type: "SET_SELECTED_BENEFITS", payload: selectedBenefits });
    console.log("Selected Benefits:", selectedBenefits);
    navigate(`/UpdateCorpDentalBenefits/${corpotateId}`);
  };
...
export default UpdateCorpBenefits;
