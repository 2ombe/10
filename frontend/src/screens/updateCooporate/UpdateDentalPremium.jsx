...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case "FETCH_SUCCESS":
      return { ...state, savedDentalData: action.payload, loading: false };
    case "FETCH_FAIL":
...
function UpdateDental() {
  ...
  const navigate = useNavigate();
    const [{ loading, error, savedDentalData}, dispatch] = useReducer(reducer, {
       loading: true,
       error: "",
     });
  ...
  const calculatedentalTotalPremiumValue = (dentalCategory) => {
   ...
   useEffect(() => {
    ...
    ctxDispatch({ type: 'SET_CORP_DENTAL', payload: dentalCorp });
  ...
  const renderCategoryTables = () => {
    return dentalCategories.map(dentalCategory => {
      const premiumData = getPremiumData(dentalCategory.selectedCategory);
      return(
