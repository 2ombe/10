...
const LowCostDetails = () => {
  ...
  const downloadPDF = async () => {
    try {
      const response = await axios.get(`/api/lowcost/${lowcostId}/download`, {
        headers: { Authorization: `Bearer ${userInfo.token}` },
        responseType: 'blob'
      });
  ...
  useEffect(() => {
    const fetchQuotation = async () => {
      try {
        const { data } = await axios.get(`/api/lowcost/single/${lowcostId}`, {
          headers: { Authorization: `Bearer ${userInfo.token}` }
        });
        setQuotation(data);
      } catch (error) {
   ...
   useEffect(()=>{
    const fetchCustomerById=async()=>{
      try {
        ...
        const customer = await axios.get(`/api/customers/${lowcostId}`)
