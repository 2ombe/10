...
const UpdateIshemaBeneficiary = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`/api/ishema/single/${ishemaId}`, {
  ...
  const handleSubmit = (e) => {
    ...
    try {
      localStorage.setItem("ishemaInfo", JSON.stringify(formData));
      dispatch({ type: "SET_ISHEMA_INFO", payload: formData });
      console.log("Form Data Saved:", formData);
      navigate(`/ishemaIn/${ishemaId}`);
    } catch (error) {
...
export default UpdateIshemaBeneficiary;
