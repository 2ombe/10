...
const UpdateIshemaBenefits = () => {
  const { state, dispatch } = useContext(AuthContext);
  const params = useParams()
  const {id:ishemaId}=params
  const navigate = useNavigate();
  const [selectedIshemaBenefits, setSelectedOptions] = useState(ishemaBenefitOptions.map(option => ({ label: option.label, value: option.value })));
  const [newBenefitInput, setNewBenefitInput] = useState("");

  useEffect(() => {
    const savedOptions = localStorage.getItem("ishemaBenefitOptions");
    if (savedOptions) {
  ...
  const handleAddNewBenefit = () => {
    if (newBenefitInput.trim() !== "") {
      ...
      setSelectedOptions([...selectedIshemaBenefits, newBenefit]);
      setNewBenefitInput("");
    }
  };

  const handleSubmit = () => {
    localStorage.setItem("ishemaBenefitOptions", JSON.stringify(selectedIshemaBenefits));
    dispatch({ type: "SET_SELECTED_ISHEMA_BENEFITS", payload: selectedIshemaBenefits });
    console.log("Selected Retail Benefits:", selectedIshemaBenefits);
    navigate(`/updateIshema/${ishemaId}`);
  };
    ...
    <div>
      ...
      <Container>
          ...
          <Col>
            <Form>
              <Form.Group controlId="benefitSelect">
                ...
                <Select
                  isMulti
                  options={ishemaBenefitOptions}
                  value={selectedIshemaBenefits}
        ...
        <Row className="my-4">
          ...
          <Col>
            <Button variant="success" onClick={() => navigate("/ishemaOut")}>
              Back
            </Button>
          </Col>
        </Row>
...
export default UpdateIshemaBenefits;
