...
const {id:ishemaId}=params
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put(`/api/ishema/ishemaQuotation/${ishemaId}`, quotationData, {
        headers: { Authorization: `Bearer ${state.userInfo.token}` },
      });
      navigate(`/ishema/${ishemaId}`);
    } catch (error) {
      console.error(error);
    }
  };
...
export default UpdateIshemaForm;
