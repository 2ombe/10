...
const AdminLowcostList= () => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const { data } = await axios.get(
        `/api/lowcost/search?CUSTOMER_NAME=${query}`,
        {
  ...
  useEffect(() => {
    const fetchQuotations = async () => {
      try {
        const {data} = await axios.get("/api/lowcost/zose",{
            headers:{Authorization: `Bearer ${userInfo.token}`}
        })
      ...
      <Form onSubmit={submitHandler} className="mb-3">
        <InputGroup>
          <FormControl
            ...
            aria-label="Search by Ccustomer Name"
          />
        ...
        <tbody>
          {quotations.map((quotation, index) => (
            <tr key={quotation._id}>
              ...
              <td>
                <Button variant="info" onClick={() => navigate(`/lowcost/${quotation._id}`)}>
                  View
                </Button>
              </td>
            </tr>
...
export default AdminLowcostList;
