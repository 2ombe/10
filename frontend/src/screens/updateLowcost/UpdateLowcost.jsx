...
function UpdateLowCost() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`/api/lowcost/lowcostQuotation/${lowcostId}`, quotationData, {
        headers: { Authorization: `Bearer ${state.userInfo.token}` },
      });
      navigate(`/lowcost/${lowcostId}`);
    } catch (error) {
