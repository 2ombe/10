...
const LowcostPendingQuotations = () => {
  ...
  useEffect(() => {
    const fetchQuotations = async () => {
      try {
        const response = await axios.get('/api/lowcost/pending', {
  ...
  const handleViewDetails = (quotationId) => {
    navigate(`/lowcost/${quotationId}`);
  };

  return (
    <div>
      <h2>Pending Lowcost Quotations</h2>
...
export default LowcostPendingQuotations
